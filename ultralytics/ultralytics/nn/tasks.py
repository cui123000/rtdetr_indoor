...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            # CBAM(c1, kernel_size=7): args[0] could be kernel_size or ignored
            # If args is empty, use default kernel_size=7
            # If args[0] is present, check if it's 3 or 7 (valid kernel_size), otherwise ignore it
            c1 = ch[f]
            if len(args) == 0:
                args = [c1]  # only c1, use default kernel_size=7
            elif len(args) == 1:
                # Check if args[0] is a valid kernel_size (3 or 7)
                if args[0] in {3, 7}:
                    args = [c1, args[0]]  # [c1, kernel_size]
                else:
                    # args[0] is not a valid kernel_size, ignore it and use default
                    args = [c1]  # only c1, use default kernel_size=7
            else:
                args = [c1, *args]
            c2 = ch[f]  # output channels same as input
        elif m is ChannelAttention:
            # ChannelAttention only needs input channels
            args = [ch[f]]
            c2 = ch[f]  # output channels same as input
        elif m in {EdgeResidual, UniversalInvertedResidual}:
            # MobileNetV4 modules: EdgeResidual(c1, c2, s, e) or UniversalInvertedResidual(c1, c2, s, e, k)
            c1 = ch[f]
            if len(args) >= 1:
                c2 = args[0]  # output channels
                args = [c1, c2, *args[1:]]  # [c1, c2, stride, expansion, kernel_size]
            else:
